...
if st.button("추천 노래 보기"):
    ...
    st.markdown(f"### 🎶 오늘의 테마: {subject} + {mood}")
    ...
    for song in picks:
        ...
        st.write(f"- [{song}]({yt_link}) — {sdesc}")
    
    # 랜덤 가사 출력
    st.markdown(f"**랜덤 가사 한 줄**: {random.choice(lyrics_snippets)}")
    
    # 오늘의 운세 출력
    st.markdown(f"**오늘의 공부 운세**: {random.choice(study_fortune)}")
